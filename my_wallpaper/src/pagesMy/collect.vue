		...
		<scroll-view class="scroll-area" scroll-y lower-threshold="120" :refresher-enabled="true"
			...
			<view class="content">
					...
					<view class="grid-item" v-for="(item, index) in list" :key="item.id" @click="goPreview(item)">
						...
						<view class="item-info">
							...
							<view class="item-meta">
								<text class="meta-num">{{ item.collectTime || '' }}</text>
							</view>
						</view>
						<view class="remove-btn" @click.stop="removeCollect(index)">
							<uni-icons type="star-filled" size="16" color="#feae2c"></uni-icons>
						</view>
					</view>
				</view>
				<view class="load-footer">
					...
					<text v-else-if="!loading && list.length === 0 && loaded" class="load-text">暂无收藏记录</text>
				</view>
			</view>
		</scroll-view>
	</view>
...
import { getUserCollects } from '@/apis/user.js'
import { collectWallpaper } from '@/apis/wallpaper.js'
import { toggleWorkCollect } from '@/apis/square.js'
...
const fetchList = async (reset = false) => {
	...
	try {
		const res = await getUserCollects(userId, {
			page: currentPage,
			pageSize,
		...
		if (res.code === 200 && res.data) {
			...
			const mapped = records.map((item) => ({
				...
				collectTime: formatDate(item.collectTime || item.createdAt || item.createTime, 'YYYY-MM-DD'),
...
const goPreview = (item) => {
	...
	if (item.dataType === 'work') {
		...
		uni.navigateTo({
			url: `/pages/preview/preview-detail?wallpaperId=${item.id}&type=${item.type || 1}&from=collect`,
			fail: (err) => {
				console.error('跳转失败:', err)
				uni.showToast({ title: '跳转失败', icon: 'none' })
			}
...
const removeCollect = async (index) => {
	const item = list.value[index]

	try {
		let res
		if (item.dataType === 'work') {
			res = await toggleWorkCollect(item.id)
		} else {
			res = await collectWallpaper(item.id)
		}
...
onLoad(() => {
	if (!isLoggedIn()) {
		redirectToLoginReplace('/pagesMy/collect')
		return
	}
	fetchList(true)
})
